...
# Temporal Embedder (Convolutional Network), Shape [B, T, D] -> [B, T * D]
class TemporalConvNet(nn.Module):
    def __init__(self, input_dim, hidden_dim = 512, T=8):
        ...
        self.tcn = nn.Sequential(
            ...
            nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, padding=2, dilation=2),    # dilation=2
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU(inplace=True),
...
class PoseEstimator(nn.Module):
    def __init__(self, num_joints=17, T=30, device='cuda'):
        ...
        self.temporal = TemporalConvNet(input_dim=self.D, T=self.T)
